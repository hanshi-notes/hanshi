...
For release packaging checks, run `Scripts/package_app.sh release` followed by `Scripts/check_preview_package.sh`. The latter temporarily hides `.build` (restoring it on exit) and renders formulas and Mermaid using the release engine objects and resources from a disposable copy of the packaged app. Run it after builds finish. Toolchain-selection regressions run with `python3 Tests/Scripts/test_swift_toolchain.py`. Native bridge tests run with `cargo test --release --locked --manifest-path Native/Mermaid/Cargo.toml --target-dir .build/mermaid`.
