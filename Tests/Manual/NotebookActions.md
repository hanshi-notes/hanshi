...
5. Click the **Hide Notebook Sidebar** button to the left of the search field. Verify it hides the notebooks and becomes **Show Notebook Sidebar**, which restores them. Verify **View → Hide/Show Notebook Sidebar** and **⌃⌘S** stay in sync with the button, and that the button is hidden in Zen mode. With the sidebar hidden, verify the button sits clear of the window's close, minimize and zoom buttons. Verify those window buttons sit vertically centred in the bar, and stay there after resizing the window and after leaving full screen. Hide the notebook sidebar again. The icon bar should show a notebook picker with **All Notes** and every notebook, including empty ones. Switch notebooks with an unsaved note: the first note opens, search clears, and returning restores the draft and undo. Show the sidebar again and verify the same notebook is selected. The picker disappears with the sidebar visible and in Zen mode.
